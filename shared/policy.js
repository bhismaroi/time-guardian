...
export function buildBreakFormula(row, dayChecks) {
...
export function buildLeaveEarlierFormula(row, dayChecks) {
...
export function buildOvertimeFormula(row, dayChecks) {
...
export function reactDayChecks(row) {
  return {
    friday: `WEEKDAY(A${row},2)=5`,
    weekday: `WEEKDAY(A${row},2)<=4`,
    weekend: `WEEKDAY(A${row},2)>5`,
...
export function cloudflareDayChecks(row) {
  return {
    friday: `B${row}="Fri"`,
    weekday: `OR(B${row}="Mon",B${row}="Tue",B${row}="Wed",B${row}="Thu")`,
    weekend: `OR(B${row}="Sat",B${row}="Sun")`,
